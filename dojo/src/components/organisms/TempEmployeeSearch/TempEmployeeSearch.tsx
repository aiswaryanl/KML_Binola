...
const TempEmployeeSearch = () => {
        ...
        if (term.length >= 2) {
            try {
                ...
                const filtered = userData.filter((user: UserInfo) =>
                    ...
                    user.tempId.toLowerCase().includes(term.toLowerCase())
                );
    ...
    const handleUserSelect = async (user: UserInfo) => {
        try {
            ...
            // Use service to check if user has existing assessment data
            const checkData = await humanBodyCheckService.checkUserAssessment(user.tempId);

            // If user has existing check data, treat them as already completed
            if (checkData && checkData.length > 0) {
                setError(`${user.firstName} has already completed the assessment`);
