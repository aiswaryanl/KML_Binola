...
interface Level3OJTStatus {
  id: number;
  traineeId?: string; // optional because some records may not have it
  trainee_name: string;
  trainer_name: string;
...
const Level3OJTStatusList: React.FC = () => {
  const [data, setData] = useState<Level3OJTStatus[]>([]);
  ...
  // Fetch data safely
  useEffect(() => {
    fetch("http://127.0.0.1:8000/level3/ojt-status/")
      .then((res) => {
        if (!res.ok) throw new Error("Failed to fetch Level 3 OJT data");
        return res.json();
      })
      .then((resData) => {
      ...
      .catch((err) => {
        console.error("Error fetching Level 3 OJT status:", err);
        setError("Unable to load Level 3 OJT data. Please try again later.");
        setLoading(false);
      });
  }, []);

  // Filtered data with safe access
        ...
        {/* <div className="mb-8 flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-gray-600 ml-14">
              Track and manage trainee OJT completion for Level 3
            </p>
          </div>
        </div> */}
          <div className="mb-8 flex items-center justify-between flex-wrap gap-4">
                  <div>
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <h1 className="text-3xl font-bold bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent">
                        {selectedView === "OJT" ? "OJT Level 3 Status" : "TenCycle Status"}
                      </h1>
                    </div>
                    <p className="text-gray-600 ml-14">
                      {selectedView === "OJT"
                  ...
                  <div className="relative">
                    ...
                    >
                      <option value="OJT">OJT Level 3 Status</option>
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredData.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-6 py-4">
                    {item.production_or_quality ? (
                      <span className="text-sm font-medium text-gray-700">
                        {item.production_or_quality} / {item.line || "Not Assigned"}
                      </span>
                    ) : (
                  ...
                  <td className="px-6 py-4">
                    {item.station || <span className="italic text-gray-400">Not Assigned</span>}
                  </td>
