...
async function fetchEmployeeDetails(empId: string) {
  const response = await fetch(`http://localhost:8000/mastertable/?emp_id=${empId}`);
...
async function fetchEmployeeEvaluations(empId: string) {
  const response = await fetch(`http://localhost:8000/productivityevaluations/?employee=${empId}`);
...
console.log(employeeInfo)
  ...
  try {
    ...
    const evaluationResponse = await fetch('http://localhost:8000/productivityevaluations/', {
      method: 'POST',
      ...
      for (const sequence of productivityModule.sequences) {
        const sequenceResponse = await fetch('http://localhost:8000/sequences/', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
          body: JSON.stringify({
      ...
      const calculateResponse = await fetch(
        `http://localhost:8000/productivityevaluations/${evaluationId}/calculate_results/`,
  ...
  const handleExport = () => {
    ...
    const exportFileDefaultName = `productivity_evaluation_${employeeInfo.name.replace(/\s+/g, '_') || 'employee'}_${
        ...
        <div className="bg-white border-2 border-black shadow-lg rounded-lg p-6">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4">
            <button
              onClick={handleSave}
              disabled={isSaving}
              className={`flex items-center space-x-2 px-6 py-3 rounded-lg transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-1 ${
                isSaving
