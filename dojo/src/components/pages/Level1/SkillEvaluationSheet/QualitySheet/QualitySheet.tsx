...
async function fetchEmployeeEvaluations(empId: string) {
  const response = await fetch(`http://localhost:8000/qualityevaluations/?employee=${empId}`);
...
function QualitySheet() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const evaluationResponse = await fetch('http://localhost:8000/qualityevaluations/', {
        method: 'POST',
      ...
      for (const sequence of qualityModule.sequences) {
        const sequenceResponse = await fetch('http://localhost:8000/qualitysequences/', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
          body: JSON.stringify({
      ...
      const calculateResponse = await fetch(
        `http://localhost:8000/qualityevaluations/${evaluationId}/calculate_results/`,
  ...
  const handleExport = () => {
    ...
    const exportFileDefaultName = `quality_evaluation_${employeeInfo.name.replace(/\s+/g, '_') || 'employee'}_${
